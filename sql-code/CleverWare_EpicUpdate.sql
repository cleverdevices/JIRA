CREATE DEFINER=`readonly`@`%` PROCEDURE `CleverWare_EpicUpdate`()
BEGIN

Drop table if exists CleverWare_EpicUpdateTable;
Create Table CleverWare_EpicUpdateTable 

select concat("TWR-",t1.epic_issuenum)  as epic_issuenum, t1.story_num, t1.issue_status,
...
union all
select concat("TWR-",t1.story_num)  as epic_issuenum, t1.story_num, t1.issue_status,
...
(select B.*, cfv.CUSTOMFIELD, cfv.numbervalue as s, cfv.stringvalue as t
 from customfieldvalue cfv join 
	...
	from jiraissue ji where issuetype in (2,3,4,5,6,7,9) and issuestatus not in (6) and project =12191) B on cfv.issue =B.story_id
where cfv.customfield in (10463)) as T1;
