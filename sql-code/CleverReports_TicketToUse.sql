CREATE DEFINER=`readonly`@`%` PROCEDURE `CleverReports_TicketToUse`()
BEGIN

call CleverReports_NRE_Tshirt_Size() ;

drop table if exists CleverReports_startInput ;
create table CleverReports_startInput
...
case when (((select CleverReports_NRE_Tshirt_Size_Table.issuestatus from CleverReports_NRE_Tshirt_Size_Table where Ticket_to_Use = CleverReports_NRE_Tshirt_Size_Table.Epic_ID) = 6) OR ((select CleverReports_NRE_Tshirt_Size_Table.issuestatus from CleverReports_NRE_Tshirt_Size_Table where Ticket_to_Use = CleverReports_NRE_Tshirt_Size_Table.Epic_ID) = 5)) Then 1 else 0 end as Ticket_Closed 


from CleverReports_Sheet2Table ;
