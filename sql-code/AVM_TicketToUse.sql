CREATE DEFINER=`readonly`@`%` PROCEDURE `AVM_TicketToUse`()
BEGIN

call AVM_NRE_Tshirt_Size() ;

drop table if exists AVM_startInput ;
create table AVM_startInput
...
case when (((select AVM_NRE_Tshirt_Size_Table.issuestatus from AVM_NRE_Tshirt_Size_Table where Ticket_to_Use = AVM_NRE_Tshirt_Size_Table.Epic_ID) = 6) OR ((select AVM_NRE_Tshirt_Size_Table.issuestatus from AVM_NRE_Tshirt_Size_Table where Ticket_to_Use = AVM_NRE_Tshirt_Size_Table.Epic_ID) = 5)) Then 1 else 0 end as Ticket_Closed 


from AVM_Sheet2Table ;
